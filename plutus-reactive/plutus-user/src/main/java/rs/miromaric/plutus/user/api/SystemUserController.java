...
public class SystemUserController {
    ...
    public Mono<ResponseEntity<SystemUser>> save(@RequestBody SystemUser systemUser) {
        return service.save(systemUser)
                .map(user -> ResponseEntity.created(URI.create("/api/v1/users/" + user.getUuid()))
                .body(user));
    }
    ...
    public Mono<SystemUser> get(@PathVariable("id") String uuid) {
        return service.get(uuid)
                .switchIfEmpty(Mono.error(SystemUserNotFoundException.withUuid(uuid)));
    }

    @PutMapping("/{id}")
    public Mono<SystemUser> update(@RequestBody SystemUser systemUser,
                                   @PathVariable("id") String uuid) {
        systemUser.setUuid(uuid);
        return service.update(systemUser)
                .switchIfEmpty(Mono.error(SystemUserNotFoundException.withUuid(uuid)));
    }
